...
class ClarioStarXlsxReader(Reader):
    def read(self, path: Path) -> MeasurementSet:
        ...
        if proto_df.shape[1] > 1 and proto_df.iloc[:, 1].notna().any():
            ...
            meta = meta_df.set_index(meta_df.columns[0])[meta_df.columns[1]]\
        ...
        tidy["channel"] = "FI"         # only one channel in our current use case, can be extended to, e.g. multiple wavelengths
