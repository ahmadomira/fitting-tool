...
def run_dba_host_to_dye_fitting(
    ...
    for replica_index, replica_data in enumerate(replicas, start=1):
        ...
        h0_uM_vector = to_uM(replica_data[:, 0])
        Signal_observed = replica_data[:, 1]
        if len(h0_uM_vector) < 2:
        ...
        for initial_params in initial_params_list:
            result = minimize(
                lambda params: np.sum(
                    residuals(
                        Signal_observed, compute_signal_dba, params, h0_uM_vector, d0_uM
                    )
                    ** 2
                ),
                initial_params,
            ...
            Signal_computed = compute_signal_dba(result.x, h0_uM_vector, d0_uM)
        ...
        if filtered_results:
            ...
            continue
        Signal_computed = compute_signal_dba(median_params, h0_uM_vector, d0_uM)
        rmse, r_squared = calculate_fit_metrics(Signal_observed, Signal_computed)
        fitting_curve_x, fitting_curve_y = [], []
        for i in range(len(h0_uM_vector) - 1):
            extra_points = np.linspace(h0_uM_vector[i], h0_uM_vector[i + 1], 21)
            fitting_curve_x.extend(extra_points)
            fitting_curve_y.extend(compute_signal_dba(median_params, extra_points, d0_uM))
        last_signal = compute_signal_dba(median_params, [h0_uM_vector[-1]], d0_uM)[0]
        fitting_curve_x.append(h0_uM_vector[-1])
        fitting_curve_y.append(last_signal)
        input_params = (d0_uM, None, None, Id_lower, Id_upper, I0_lower, I0_upper)
