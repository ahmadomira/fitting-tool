...
def run_gda_merge_fits(results_dir, outlier_relative_threshold, rmse_threshold_factor, kg_threshold_factor, save_plots, display_plots, save_results, results_save_dir):
    # Load replica data
    ...
    fig1, ax1 = create_plots(x_label=r'$D_0$ $\rm{[\mu M]}$', y_label=r'Signal $\rm{[AU]}$', plot_title='All Replica Data and Fitting Curves with Outlier Detection')
    ...
    # Filter replicas based on RMSE and Kg thresholds
    valid_replicas = []
    for idx, replica in enumerate(replicas):
        median_params = [
            replica['median_params']['I0'],
    ...
    fig2, ax2 = create_plots(x_label=r'$D_0$ $\rm{[\mu M]}$', y_label=r'Signal $\rm{[AU]}$', plot_title='Averaged Data and Fitting Curve')
...
class GDAMergeFitsApp:
    ...
    def run_merge_fits(self):
        try:
            ...
            run_gda_merge_fits(
                results_dir, outlier_relative_threshold, rmse_threshold_factor,
                kg_threshold_factor, save_plots, display_plots, save_results, results_save_dir
            )
